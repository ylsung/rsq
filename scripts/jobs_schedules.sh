...
do
    ...
    for w_bits in 3 4
    do
        for scheduler in rlinear linear
        do
            for seed in ${seeds[@]}
            do
                q_type=quarot_${w_bits}bit_w_n${nsamples}_l${train_seqlen}_${scheduler}@${seed}
                ...
                --rotate \
                --seed ${seed} \
                --scheduler_yaml fake_quant/configs/${scheduler}.yaml \
                --nsamples ${nsamples} \
