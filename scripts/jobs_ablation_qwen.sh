source /home/ylsung/codes/env.sh

nsamples_list=(1 2 4 8 16 32 64 128 256 512 1024 2048 4096)
train_seqlen_list=(1048576 524288 262144 131072 65536 32768 16384 8192 4096 2048 1024 512 256)

# 3 to 12
# 3 6 9 12
nsamples_list=(128 256 512 1024 2048 8192 4096 16384 32768)
train_seqlen_list=(8192 4096 2048 1024 512 128 256 64 32)
val_size_list=(16 32 64 128 256 1024 512 2048 4096)

declare -a jobs

seeds=(0 1 2)

source scripts/additional_eval_func_qwen.sh

i=1
train_seqlen=${train_seqlen_list[i]}
nsamples=${nsamples_list[i]}
val_size=0

num_bins=0.005
attn_weighting=org_attn_none

model_size=7
path_prefix=qwen-2.5-${model_size}B-instruct
full_model_name=Qwen/Qwen2.5-${model_size}B-Instruct

for w_bits in 3
do
    for seed in 0 1 2
    do
        q_type=${w_bits}bit_w_n${nsamples}_l${train_seqlen}_${attn_weighting}1_min${num_bins}@${seed}
        save_name=${path_prefix}_${q_type}
        # no rotate

        job="eval cd /playpen-ssd/ylsung/quant_repo_everything; \
        python fake_quant/main.py \
        --model ${full_model_name} \
        --w_bits ${w_bits} --w_clip \
        --seed ${seed} \
        --val_size ${val_size} \
        --min_value ${num_bins} \
        --max_value 1 \
        --add_until_fail \
        --module_input_weighting_yaml fake_quant/configs/input_weighting/${attn_weighting}.yaml \
        --nsamples ${nsamples} \
        --train_seqlen ${train_seqlen} \
        --wandb_project QuaRot \
        --wandb_id ylsung \
        --save_name ${save_name} \
        --lm_eval \
        --save_qmodel_path /playpen-ssd2/ylsung/checkpoints/${save_name}.pth \
        --tasks piqa hellaswag arc_easy arc_challenge winogrande lambada"

        echo $job
        jobs+=("$job")
    done
done

for w_bits in 3
do
    for seed in 0 1 2
    do
        q_type=${w_bits}bit_w_n${nsamples}_l${train_seqlen}_${attn_weighting}1_min${num_bins}@${seed}
        save_name=${path_prefix}_${q_type}
        # no rotate
        
        mapfile new_jobs < <(add_additional_jobs "${full_model_name}" "${path_prefix}" ${save_name} False)

        for item in "${new_jobs[@]}"; do
            echo $item
        done

        jobs+=("${new_jobs[@]}")
    done
done

