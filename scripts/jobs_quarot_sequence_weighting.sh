...
do
    ...
    do
        ...
        for w_bits in 3
        do
            for attn_weighting in sequence_input_magnitude sequence_output_magnitude
            do
                for seed in 0 1 2
                do
                    q_type=quarot_${w_bits}bit_w_n${nsamples}_l${train_seqlen}_${attn_weighting}_none${num_bins}@${seed}
                    ...
                    --val_size ${val_size} \
                    --rotate \
                    --feature_max_value ${num_bins} \
                    --add_until_fail \
                    --layerwise_weighting \
                    --sequence_weighting_yaml fake_quant/configs/sequence_weighting/${attn_weighting}.yaml \
                    --nsamples ${nsamples} \
    ...
    do
        ...
        do
            ...
            do
                for seed in 0 1 2
                do
                    q_type=quarot_${w_bits}bit_w_n${nsamples}_l${train_seqlen}_reverse_${attn_weighting}_none${num_bins}@${seed}
                    save_name=llama3-8b-instruct_${q_type}
                    # no rotate
