...
def round_ste(x: torch.Tensor):
    # borrowed from https://github.com/OpenGVLab/EfficientQAT/blob/main/quantize/quantizer.py
    """
...
class QATQuantizedWeights(torch.nn.Module):
    ...
    def forward(self):
        ...
        if self.zero is not None:
            zero = self.zero.to(self.weight_fp.device)
            q = clamp_ste(round_ste(self.weight_fp / scale) + zero, 0, self.maxq)
            return (scale * (q - zero)).to(self.dtype)
        
        q = clamp_ste(round_ste(self.weight_fp / scale), -(self.maxq+1), self.maxq)
